...
class TestGetActions(unittest.TestCase):
        ...
        assert (
            GetAndCleanUp(
                ...
                cleanup_actions=[
                    ...
                    for region in self.resolver.boto3_services.enabled_regions
                ],
            )
            in result
        )
    ...
    def test_single_resource_regional(self):
        result = self.resolver.get_actions(resource_types=["instance"])
        expected_result = [
            GetAndCleanUp(
    ...
    def test_single_resource_global_service_regional_resource(self):
        assert self.resolver.get_actions(resource_types=["bucket"]) == [
            GetAndCleanUp(
                get_actions=[GetAction(service_name="s3", region="us-east-1", resource_type="bucket")],
                cleanup_actions=[
                    CleanupAction(service_name="s3", region=region, resource_type="bucket")
    ...
    def test_single_resource_global_service_non_api_region(self):
        assert self.resolver.get_actions(resource_types=["role"], regions=["eu-west-2"]) == []

    def test_single_resource_global_service_regional_resource_api_region(self):
        assert self.resolver.get_actions(resource_types=["bucket"], regions=["us-east-1"]) == [
            GetAndCleanUp(
                get_actions=[GetAction(service_name="s3", region="us-east-1", resource_type="bucket")],
                cleanup_actions=[
                    CleanupAction(service_name="s3", region=region, resource_type="bucket")
    ...
    def test_single_resource_global_service_regional_resource_non_api_region(self):
        assert self.resolver.get_actions(resource_types=["bucket"], regions=["eu-west-2"]) == []

    def test_exclude_resources(self):
        result = self.resolver.get_actions(service_names=["ec2"], exclude_resources=["ec2:instance"])
