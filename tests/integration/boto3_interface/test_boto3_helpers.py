...
class TestBoto3Getter(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        get_default_mocker().start_general_mock(restrict_services=False, restrict_collections=False)
        add_infra()
        cls.custom_resource_definitions = CustomResourceDefinitions(DEFAULT_SESSION)
        cls.boto3_resources = {
            "iam:role": cls.custom_resource_definitions.resource("iam").Role("test-role"),
            "ec2:vpc": next(iter(cls.custom_resource_definitions.resource("ec2").vpcs.all())),
        }
    ...
    def setUp(self):
        ...
        self.boto3_getter = Boto3Getter(DEFAULT_SESSION, service_maps=self.service_maps)
    ...
    def test_get_resource_from_collection(self):
        result = list(
            get_resource_from_collection(
                ...
                boto3_resource_collection=self.boto3_service.meta.resource_model.collections[0],
            )
        )
    ...
    def test_get_subresources(self):
        result = list(self.boto3_getter.get_subresources(boto3_resource=self.boto3_resources["iam:role"]))

        assert result[0].name == "test-role-policy"

    def test_get_child_resources_subresources(self):
        result = list(
            self.boto3_getter.get_child_resources(
    ...
    def test_get_secondary_attributes(self):
        result = list(self.boto3_getter.get_secondary_attributes(boto3_resource=self.boto3_resources["ec2:vpc"]))
    ...
    def test_get_child_resource_definitions_subresources(self):
        result = list(
            self.boto3_getter.get_child_resource_definitions(
                service_name="iam",
                boto3_resource_model=self.boto3_resources["iam:role"].meta.resource_model,
                resource_type="resource",
            )
        )
    ...
    def test_get_child_resource_definitions_secondary_attributes(self):
        result = list(
            self.boto3_getter.get_child_resource_definitions(
                service_name="ec2",
                boto3_resource_model=self.boto3_resources["ec2:vpc"].meta.resource_model,
                resource_type="secondaryAttribute",
            )
        )
...
class TestBoto3Helpers(unittest.TestCase):
    ...
    def test_get_resource_collection_by_resource_type(self):
        result = get_resource_collection_by_resource_type(boto3_service=self.boto3_service, resource_type="role")

        assert isinstance(result, Collection)

    def test__get_resource_attributes(self):
        result = _get_resource_attributes(boto3_resource=self.boto3_resources["iam:role"])
    ...
    def test_get_service_resource_types_from_collections(self):
        result = list(
            get_service_resource_types_from_collections(collections=self.boto3_service.meta.resource_model.collections)
        )
