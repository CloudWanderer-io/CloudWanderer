...
from cloudwanderer.storage_connectors.dynamodb import Attr, DynamoDbQueryGenerator, IndexNotAvailableException, Key
...
class TestDynamoDbQueryGenerator(unittest.TestCase):
    ...
    def test_account_id_and_service_and_resource_type(self):
        ...
        assert qg.index == "resource_type"
        assert next(qg.condition_expressions) == Key("_resource_type_index").eq("ec2#vpc#shard0") & Key(
            "_resource_type_range"
        ).begins_with("111111111111#")
        assert qg.filter_expression == Attr("_account_id").eq("111111111111") & Attr("_service").eq("ec2") & Attr(
            "_resource_type"
        ).eq("vpc")
